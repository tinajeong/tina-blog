...
예를 들어, 파일의 메시지 ID를 추출하는 코드가 있다. 아래 코드는 윈도우 환경에서 잘 동작한다.

```java
private Optional<String> extractMessageID( File f )  
{   
    int startPos = 
        f.getPath().lastIndexOf('\\');  // Windows path separator
    int endPos = 
        f.getPath().lastIndexOf(CharacterEnum.PERIOD.getChar() );  
...
위 예시 코드는 윈도우에서 잘 작동하지만 맥북에서 돌려보면 에러가 발생한다. 
...
{   
    int startPos = 
        f.getPath().lastIndexOf(System.getProperty("file.separator"));  // 한 줄 수정하기
    int endPos = 
        f.getPath().lastIndexOf(CharacterEnum.PERIOD.getChar() );  
...
nio 패키지의 Path를 사용하면 운영체제에 따른 파일 경로 구분자 식별 문제를 해결할 수 있다. 또한, `startPos`를 찾는 부분이 필요 없어져 코드가 더 간결해진다.

```java
private Optional<String> extractMessageID( Path path )
{
    int endPos = path.getFileName().toString().lastIndexOf( CharacterEnum.PERIOD.getChar() );
    if( endPos < 0 )
        return Optional.empty();
    return Optional.of(path.getFileName().toString().substring( 0, endPos ));
}
```
