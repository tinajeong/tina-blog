...
defer 옵션으로 js를 로딩한 상태에서 파일 상위에 초기화한 todo array가 null 로 떠서 해당 에러가 나는 줄에 초기화 코드를 넣어주니 해결되었다. 이전에 동일한 로직으로, [한 js파일에서 개발한 코드](https://gist.github.com/tinajeong/323fdf994723a45d27433a9cb632be59)에서는 잘 돌아갔는데, 로딩시점의 문제인지 추측만 하는 중이다. 
