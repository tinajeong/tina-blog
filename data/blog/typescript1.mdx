...
반면 typescript는 컴파일타임에 타입을 검사하는 static type 전략을 가지고 있기 때문에 **컴파일 타임의 타입검사**가 또다시 타입스크립트의 장점으로 강조된다. 
...
object의 속성 타입을 지정할 수 있다. 물론 위 코드처럼 객체타입을 바로 기술하는 형태는 자주 사용되는것은 아니다. 
...
```
`void`를 통해 함수의 반환값이 없음을 명시할 수 있다. 자바도 가지고 있는 feature니 놀라울 것은 없지만, 자바스크립트에선 `void`가 없었기 때문에 기억할 필요가 있다.
```typescript
...
```
### First Class Citizen
함수의 파라미터로 넘어가고, 함수의 반환값이 되고, 변수에 담길 수 있다면  First Class Citizen으로 분류된다. 
