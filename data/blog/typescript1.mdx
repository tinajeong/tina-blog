...
# 1. 타입스크립트의 필요성
C, C++, Java 등은 강타입 언어이다. 프로그래머가 변수의 타입을 명시할 것을 강제한다. 
타입의 부재로 인한 여러가지 문제의 가능성을 compile 타임에 차단해준다.  

문법 검사 기능을 제공하는 IDE나 에디터에서 문제를 인식하고 불평할 수 있다.
...
```
타입스크립트는 자바스크립트의 **Super Set**으로서, 타입명시는 물론이고 제너릭/인터페이스 등 다른 언어에서 지원하는 특성들을 사용할 수 있다. 
...
반면 typescript는 컴파일타임에 타입을 검사하는 static type 전략을 가지고 있기 때문에 컴파일 타임의 검사가 또다시 타입스크립트의 장점으로 강조된다. 
...
즉, 정수 5와 실수 5.0이 타스에서는 동일하게 인식된다. 
...
## object 
자바스크립트에서는 없는 요소에 대해 undefined으로 표시하지만, 타입스크립트에서는 요소의 정의와 존재여부를 검사해준다. 
```javascript
const person = {
	name: "John",
	age: 21
...
```
자바 객체의 최상위 객체가 Object인 것처럼, object도 그러하다. 
object의 속성 타입을 지정할 수 있다. 물론 아래의 형태는 자주 사용되는것은 아니다. 객체와 인스턴스를 사용하는 방식은 뒤에서 다룬다.
```typescript
...
```
## array
배열은 요소의 타입이 동일해야 한다. 물론 `any`를 사용할 수 있지만, 타입스크립트에서 일반적인 방식은 아니다. 아래의 코드는 타입 추론을 통해 
hobbies 배열의 타입을 `string[]`으로 인식하고 있다.
```typescript
const person = {
	name: "John",
	age: 21,
...
그렇지만, `any` 타입의 사용을 추천하지 않는다! 타입스크립트 컴파일러가 제공하는 타입 검사의 기능을 누리지 못하게 되기 때문이다.
...
```
`void`를 통해 함수의 반환값이 없음을 명시할 수 있다. 자바도 가지고 있는 feature니 놀라울 것은 없지만, 자바스크립트에선 없었기 때문에 기억할 필요가 있다.
```typescript
...
```
이때, void 타입의 함수를 undefined로 반환한다. 더불어, undefined로 반환타입을 명시했다면 반드시 return문이 있어야 한다.
특이하다. 물론, undefined를 반환타입으로 사용하는 것은 흔치 않다고 한다.
```typescript
...
```
### First Class Citizen
함수의 파라미터로 넘어가고, 함수의 반환값이 되고, 변수에 담길 수 있다면  First Class CItizen으로 분류된다. 함수가 일급이 될 수 있냐에 따라 함수형 프로그래밍과 연관성이 달라진다. 자바스크립트에선 가능한 반면, Java에서는 함수를 First Class Citizen으로 보지 않는다. 
...
자바는 객체 지향 언어이지만, 함수형프로그래밍을 지원하기 위한 도구로 Functional Interface를 지원한다. 
```java
... 
